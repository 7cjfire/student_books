...
├── file-service/         8086   文件上传（阿里云 OSS / 本地磁盘）+ EasyExcel 批量导入分类
...
默认 `oss.enabled=false`，即走本地磁盘（`./uploads/`），方便本地联调。切到 OSS 时推荐用环境变量注入，**不要把 AccessKey 写到 yml 里提交到仓库**：
...
- [x] file-service（阿里云 OSS + 本地兜底 + EasyExcel 导入 edu_subject）
